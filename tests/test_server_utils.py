...
def test_check_cuda(monkeypatch):
    ...
    def dummy_subprocess_fn_factory(
        output="", error="", returncode=0, raise_exc=False
    ):
    ...
    monkeypatch.setattr(
        compox.server_utils,
        "get_subprocess_fn",
        lambda *args, **kwargs: dummy_subprocess_fn_factory(
            output="0, GPU0, 1024\n1, GPU1, 2048",
            error="",
            returncode=0,
            raise_exc=True,
    ...
    assert (
        avail is True
    ), f"Expected 'True' when nvidia-smi succeeds, got {avail!r}"
    ...
    assert (
        avail is True
    ), f"Expected True when falling back to nvidia-smi, got {avail!r}"
...
def test_check_mps(monkeypatch):
    ...
    assert (
        mps_avail is False
    ), f"Expected 'false' when torch is missing, got {mps_avail!r}"
    ...
    assert (
        mps_avail is True
    ), f"Expected 'true' when MPS is available, got {mps_avail!r}"
    ...
    assert (
        mps_avail is False
    ), f"Expected 'false' when MPS is unavailable, got {mps_avail!r}"
...
def test_weak_lru():
    ...
    assert (
        d1.count == 1
    ), f"Expected count to be '1' on first call, got {d1.count!r}"
    ...
    assert (
        d1.count == 1
    ), f"Expected count to be '1' when cache hit, got {d1.count!r}"

    # 2) Try to add another item
    assert (
        d1.add(1) == 2
    ), f"Expected '2' when adding a new item, got {result!r}"
    assert (
        d1.count == 2
    ), f"Expected count to be '2' after adding a new item, got {d1.count!r}"
    ...
    assert (
        d2.count == 1
    ), f"Instance d2 should have its own cache; count should be 1, got {d2.count!r}"
    assert (
        d3.count == 1
    ), f"Instance d3 should have its own cache; count should be 1, got {d3.count!r}"
    ...
    assert (
        ref() is None
    ), f"After deletion and GC, weakref should be 'None', got {ref()!r}"
...
def test_algorithm_cache():
    ...
    assert calc.calls == [
        (1, 2)
    ], f"After first miss: expected calls to be '[(1,2)]', got {calc.calls!r}"
    ...
    assert calc.calls == [
        (1, 2)
    ], f"Cache hit should not append: got {calc.calls!r}"

    # fill cache with two new keys
    assert (
        calc.add(2, 3) == 5
    ), f"Second unique key: expected '5', got {calc.calls[-1]!r}"
    assert (
        calc.add(3, 4) == 7
    ), f"Third unique key: expected '7', got {calc.calls[-1]!r}"
    assert calc.calls == [
        (1, 2),
        (2, 3),
        (3, 4),
    ], f"Expected call log to be '[(1,2),(2,3),(3,4)]', got {calc.calls!r}"

    # eviction of (1,2)
    result = calc.add(1, 2)
    assert (
        result == 3
    ), f"After eviction, expected add(1,2) to return 3, got {result!r}"
    assert calc.calls[-1] == (
        1,
        2,
    ), f"Expected last call to be '(1,2)' after eviction, got {calc.calls[-1]!r}"
...
def test_data_cache():
    ...
    assert (
        result == "value_a"
    ), f"First load: expected 'value_a', got {result!r}"
    assert loader.calls == [
        "a"
    ], f"After first load: expected calls '['a']', got {loader.calls!r}"

    # same key → cache hit
    result = loader.load("a")
    assert (
        result == "value_a"
    ), f"Cache hit: expected 'value_a' again, got {result!r}"
    assert loader.calls == [
        "a"
    ], f"Cache hit should not append: got {loader.calls!r}"

    # new key → eviction of "a"
    result = loader.load("b")
    assert (
        result == "value_b"
    ), f"New key load: expected 'value_b', got {result!r}"
    assert loader.calls == [
        "a",
        "b",
    ], f"Expected calls '['a','b']', got {loader.calls!r}"

    # after eviction, "a"
    result = loader.load("a")
    assert (
        result == "value_a"
    ), f"After eviction, expected 'value_a' for 'a', got {result!r}"
    assert (
        loader.calls[-1] == "a"
    ), f"Expected last call to be 'a', got {loader.calls[-1]}"
...
def test_zip_importer(tmp_path):
    ...
    with ZipImporter(data, modname) as m:
        assert hasattr(
            m, "VALUE"
        ), "Expected imported module to have attribute 'VALUE'"
...
def test_check_and_create_database_collections():
    ...
    assert db.created == [
        "c2"
    ], f"Expected db.created to be '['c2']', got {db.created!r}"


# Test 8 - get_subprocess_fn
@pytest.mark.skipif(
    sys.platform != "posix", reason="Test will not succeed on non-posix OS."
)
def test_get_subprocess_fn_posix(monkeypatch):
    ...
    assert (
        get_subprocess_fn() == subprocess.Popen
    ), f"Expected 'subprocess.Popen' on posix, got {get_subprocess_fn()!r}"
...
def test_get_subprocess_fn_nt(monkeypatch):
    ...
    ), f"Expected a 'functools.partial' on nt, got {type(fn)!r}"
    assert (
        fn.func == JobPOpen
    ), f"Expected fn.func to be 'JobPOpen', got {fn.func!r}"


# Test 10 - get_subprocess_fn
@pytest.mark.skipif(
    sys.platform == "posix" or sys.platform == "win32",
    reason="Test is only meant for unsupported OS.",
)
