...
from t3d_server.algorithm_utils.io_schemas import ImageSchema, EmbeddingSchema
...
class Image2EmbeddingRunner(BaseRunner):
    ...
    def post_data(self, data: list[dict]) -> list[str]:
        ...
        Runner type. In this case, the schema is EmbeddingSchema.
        ...
        """
        return self.task_handler.post_data(data, EmbeddingSchema)
    ...
    def postprocess(self, data: np.ndarray, args: dict = {}) -> list[str]:
        ...
        output_dicts = [{"features": data[i]} for i in range(data.shape[0])]
