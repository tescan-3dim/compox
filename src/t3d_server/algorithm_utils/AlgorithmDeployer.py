...
from t3d_server.algorithm_utils.AlgorithmConfigSchema import (
...
class AlgorithmDeployer:
    ...
    def __init__(
        ...
        # check if ["tool"]["t3d_server"] exists in pyproject.toml
        if "tool" in pyproject_toml and "t3dserver" in pyproject_toml["tool"]:
            algorithm_config = pyproject_toml["tool"]["t3dserver"]
            ...
            warnings.warn(
                (
                    "The [tool.t3dserver] section is not found in pyproject.toml."
                    ...
                    "additional parameters, add the [tool.t3dserver] section to the"
        ...
        self.check_importable = pyproject_toml["tool"]["t3dserver"][
            "check_importable"
        ]
        self.obfuscate = pyproject_toml["tool"]["t3dserver"]["obfuscate"]
        self.hash_module = pyproject_toml["tool"]["t3dserver"]["hash_module"]
        self.hash_assets = pyproject_toml["tool"]["t3dserver"]["hash_assets"]
    ...
    def store_algorithm(
        ...
        separate_runner_path: str = None,
        algorithm_collection_name="algorithm-store",
        module_collection_name="module-store",
        asset_collection_name="asset-store",
    ) -> str:
        """
        ...
        separate_runner_path : str, optional
    ...
    def _rename_folders_and_file_with_unique_ids(
        ...
        py_files_with_relative_path : list
            The list of .py files with their relative path.
    ...
    @staticmethod
    def find_py_files(directory: str, ignore_pycache=True) -> list[str]:
        """
    ...
    def find_other_than_py_files(
        directory: str, ignore_pycache=True, ignore_gitignore=True
    ) -> list[str]:
        """
    ...
    def _rename_all_files(
        self, module_path: str, files_with_relative_paths, mode="uuid"
    ) -> dict:
        """
